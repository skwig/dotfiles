...
//    property int savedGifIndex: 0
    ...
    function refreshWifi() {
        ...
        if (!wifiTriggerScanProc.running) wifiTriggerScanProc.running = true
...
//    }
    ...
    Process {
        id: wifiStatusProc
        command: ["bash", "-c", "iwctl device wlan0 show | grep -q 'Powered.*on' && echo 'enabled' || echo 'disabled'"]
        stdout: SplitParser { onRead: data => root.wifiEnabled = data.trim() === "enabled" }
    }

    Process {
        id: wifiCurrentProc
        command: ["bash", "-c", "iwctl station wlan0 show | awk '/Connected network/ {sub(/^[ \t]*Connected network[ \t]*/, \"\"); sub(/[ \t]+$/, \"\"); ssid=$0} /RSSI/ {rssi=$(NF-1)} END {if (ssid!=\"\") print \"yes:\"ssid\":\"rssi; else print \"no::0\"}'"]
        stdout: SplitParser {
            onRead: data => {
                var parts = data.trim().split(":")
                if (parts.length >= 3 && parts[0] === "yes") {
                    root.wifiCurrentSSID = parts[1].replace(/\u001b\[[0-9;]*[mGK]/g, "").replace(/^\s*>\s*/, "").trim()
                    var rssi = parseInt(parts[2]) || -100
                    root.wifiSignal = Math.min(100, Math.max(0, 2 * (rssi + 100)))
                } else {
    ...
    Process {
        id: wifiTriggerScanProc
        command: ["bash", "-c", "iwctl station wlan0 scan"]
        onExited: {
    ...
    Process {
        id: wifiToggleProc
        command: ["bash", "-c", root.wifiEnabled ? "iwctl device wlan0 set-property Powered off" : "iwctl device wlan0 set-property Powered on"]
        onExited: {
            if (!wifiStatusProc.running) wifiStatusProc.running = true
    ...
    Process {
        ...
        command: ["bash", "-c", "iwctl station wlan0 disconnect"]
        onExited: {
            root.wifiCurrentSSID = ""
            root.wifiSignal = 0
        }
